"""Run on Nicole's Ubuntu desktop after installation."""
import json
import time
...
tool = request("/v1/chat/completions", {
    ...
    "messages": [{
        "role": "user",
        "content": "Call read_file to read README.md. Do not describe it; use the tool.",
    }],
    "tools": [{
        "type": "function",
        "function": {
...
assert model.get("size_vram", 0) >= model.get("size", 1), (
    "Partial GPU offload: inspect ollama ps"
)
