"""Loopback Ollama gateway for the standalone Ubuntu setup."""
import asyncio
import json
import os
...
def constrain(path, data):
    ...
    if path.startswith("/api/"):
        ...
        # Do not force num_ctx here. Each tuned Ollama alias carries its own
        # native context window in its Modelfile.
        ...
        data["max_tokens"] = min(
            bounded(data.get("max_tokens"), TOKENS),
    ...
    try:
        ...
        async with asyncio.timeout(DEADLINE):
            async with request.app["client"].request(
                request.method,
                BACKEND + request.rel_url.path_qs,
                data=body or None,
        ...
        raise web.HTTPGatewayTimeout(
            text="Local generation deadline reached or backend unavailable."
        ) from exc
...
if __name__ == "__main__":
    web.run_app(
        make_app(),
        host="127.0.0.1",
        port=int(os.getenv("AI_PORT", "11435")),
