...
import gateway
from gateway import constrain, TOKENS, CONTEXT


class Limits(unittest.TestCase):
    def test_native_overrides_unlimited_and_thinking(self):
        result = constrain('/api/chat', {'think': True, 'options': {'num_predict': -1, 'num_ctx': 999999}})
        self.assertFalse(result['think'])
        self.assertEqual(result['options']['num_predict'], TOKENS)
        self.assertEqual(result['options']['num_ctx'], CONTEXT)

    def test_openai_caps_both_output_fields(self):
        result = constrain('/v1/chat/completions', {'max_tokens': 99999, 'max_completion_tokens': 128,
                                                  'reasoning': {'effort': 'high'}, 'n': 10})
    ...
    def test_smaller_limit_and_tools_preserved(self):
        ...
        result = constrain('/v1/chat/completions', {'max_tokens': 32, 'tools': tools})
    ...
    def test_invalid_limits_cannot_disable_cap(self):
        for value in [None, -1, 0, True, 'unlimited', 1.5]:
            self.assertEqual(constrain('/api/chat', {'options': {'num_predict': value}})['options']['num_predict'], TOKENS)
...
class HTTPBehavior(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.patch = patch.object(gateway, 'BACKEND', str(self.backend.make_url('')).rstrip('/'))
        self.patch.start()
    ...
    async def test_stream_passes_through_and_is_constrained(self):
        response = await self.client.post('/v1/chat/completions', json={'stream': True, 'reasoning_effort': 'high'})
    ...
    async def test_deadline_returns_gateway_timeout(self):
        with patch.object(gateway, 'DEADLINE', 0.02):
            response = await self.client.post('/v1/chat/completions', json={'slow': True})
